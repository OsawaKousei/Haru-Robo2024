...
class ActionDetectNode : public rclcpp::Node {
public:
    ...
    explicit ActionDetectNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("action_detect_node", options)
    {
        ...
        this,
        "manip_control",
        std::bind(&ActionDetectNode::handle_goal, this, _1, _2),
        std::bind(&ActionDetectNode::handle_cancel, this, _1),
        std::bind(&ActionDetectNode::handle_accepted, this, _1));
    }
private:
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleState> goal_handle)
  {
    ...
    std::thread{std::bind(&ActionDetectNode::execute, this, _1), goal_handle}.detach();
  }
...
int main(int argc, char *argv[]) {
    ...
    rclcpp::spin(std::make_shared<ActionDetectNode>());
    rclcpp::shutdown();
    return 0;
}
